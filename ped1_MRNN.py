...
def test_binary(multiple_out=False,n_epochs=250):
    ...
    for seq_num in range(0,180):
        guess = model.predict(test_seq[seq_num])
        np.savetxt(f2,guess,delimiter=",")
    f2.close()
    f3=file(path2,"r")
    gs=np.loadtxt(f3,delimiter=",")
    f3.close()
    f4=file(path1,"r")
    gv=np.loadtxt(f4,delimiter=",")
    f4.close()
    gv=gv.reshape(-1,1104)
    path3='distance_MRNN.csv'
    f5=file(path3,"a")
    distance=(gs-gv)**2
    np.savetxt(f5,distance,delimiter=",")
    f5.close()
...
if __name__ == "__main__":
    ...
    test_binary(multiple_out=True,n_epochs=1500)
    aggregation()
