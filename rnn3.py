...
class RNN(object):
    ...
    """
    def __init__(self, input, n_in, n_mul, n_out, activation=T.tanh,
        ...
        W_init = np.asarray(np.random.uniform(size=(78*n_mul, 78*n_mul),
                                              low=-.01, high=.01),
                                              dtype=theano.config.floatX)
        self.W = theano.shared(value=W_init, name='W')
        # input to hidden layer weights
        W_in1_init = np.asarray(np.random.uniform(size=(1104, 78*n_mul),
                                                 low=-.01, high=.01),
                                                 dtype=theano.config.floatX)
        
        self.W_in1 = theano.shared(value=W_in1_init, name='W_in1')
        mark=np.zeros_like(W_in1_init,dtype=theano.config.floatX)
        for k in range(0,n_mul):
            for i in range(0,46):
                mark[24*i:24*(i+1),i+46*k]=1
        for k in range(0,n_mul):
            j=0
            for i in range(0,46):
                if i in range(0,3):
                    mark[24*i:24*(i+2),j+46*n_mul+32*k]=1
                    mark[24*(5+i):24*(7+i),j+46*n_mul+32*k]=1
                    j+=1
                if i in range(4,8):
                    mark[24*i:24*(i+2),j+46*n_mul+32*k]=1
                    mark[24*(6+i):24*(8+i),j+46*n_mul+32*k]=1
                    j+=1
                if i in range(9,14):
                    mark[24*i:24*(i+2),j+46*n_mul+32*k]=1
                    mark[24*(7+i):24*(9+i),j+46*n_mul+32*k]=1
                    j+=1
                if i in range(15,21):
                    mark[24*i:24*(i+2),j+46*n_mul+32*k]=1
                    mark[24*(8+i):24*(10+i),j+46*n_mul+32*k]=1
                    j+=1
                if i in range(22,29):
                    mark[24*i:24*(i+2),j+46*n_mul+32*k]=1
                    mark[24*(8+i):24*(10+i),j+46*n_mul+32*k]=1
                ...
                if i in range(30,37):
                    ...
                    mark[24*(8+i):24*(10+i),j+46*n_mul+32*k]=1
                    j+=1
        W_in4_init = np.asarray(np.random.uniform(size=(16*4, 32*n_mul),
                                                 low=-.01, high=.01),
                                                 dtype=theano.config.floatX)
        self.W_in4 = theano.shared(value=W_in4_init, name='W_in4')
        W_in9_init = np.asarray(np.random.uniform(size=(16*9, 20*n_mul),
                                                 low=-.01, high=.01),
                                                 dtype=theano.config.floatX)
        self.W_in9 = theano.shared(value=W_in9_init, name='W_in9')
        W_in16_init = np.asarray(np.random.uniform(size=(16*16, 10*n_mul),
                                                 low=-.01, high=.01),
                                                 dtype=theano.config.floatX)
        self.W_in16 = theano.shared(value=W_in16_init, name='W_in16')
        
        # hidden to output layer weights
        W_out_init = np.asarray(np.random.uniform(size=(78*n_mul, n_out),
                                                  low=-.01, high=.01),
                                                  dtype=theano.config.floatX)
        self.W_out = theano.shared(value=W_out_init, name='W_out')
        '''
        markoutput=np.zeros_like(W_out_init,dtype=theano.config.floatX)
        content=[[[],[],[],[],[1,47],[2,48],[3,49],[4]],[[],[],[],[5,50],[6,51],\
        [7,52],[8,53],[9]],[[],[],[10,54],[11,55],[12,56],[13,57],\
        [14,58],[15]],[[],[16,59],[17,60],[18,61],[19,62],[20,63],\
        [21,64],[22]],[[23,65],[24,66],[25,67],[26,68],[27,69],\
        [28,70],[29,71],[30]],[[31,72],[32,73],[33,74],[34,75],[35,76],[36,77],\
        [37,78],[38]],[[39],[40],[41],[42],[43],[44],[45],[46]]]
        b=0
        for i in range(0,7):
            for j in range(0,8):
                for k in range(0,2):
                    for h in range(0,2):
                        if len(content[i][j])>0:
                            if k==0 and h==0:
                                for n in range(0,n_mul):
                                    for m in range(0,len(content[i][j])):
                                        markoutput[n*78+content[i][j][m]-1][b]=1
                                    
                            elif i-k>=0 and j-h>=0:
                                if len(content[i-k][j-h])>1:
                                    for n in range(0,n_mul):
                                        markoutput[n*78+content[i-k][j-h][1]-1][b]=1
                if len(content[i][j])>0:
                    b+=1
        '''
        h0_init = np.zeros((78*n_mul,), dtype=theano.config.floatX)
        self.h0 = theano.shared(value=h0_init, name='h0')

        bh_init = np.zeros((78*n_mul,), dtype=theano.config.floatX)
        self.bh = theano.shared(value=bh_init, name='bh')

        by_init = np.zeros((n_out,), dtype=theano.config.floatX)
        self.by = theano.shared(value=by_init, name='by')

        self.params = [self.W, self.W_in1,self.W_out, self.h0,
                       self.bh, self.by]
        ...
        '''
            for k in range(0,n_mul):
                ...
                for i in range(0,46):
                    if i in range(0,3):
                        tt=T.concatenate([x_t[24*i:24*(i+2)],x_t[24*(5+i):24*(7+i)]])
                        h_tm=T.concatenate([h_tm,T.dot(tt,self.W_in4[:,k*32+j]).dimshuffle('x')])
                        j+=1
                    if i in range(4,8):
                        tt=T.concatenate([x_t[24*i:24*(i+2)],x_t[24*(6+i):24*(8+i)]])
                        h_tm=T.concatenate([h_tm,T.dot(tt,self.W_in4[:,k*32+j]).dimshuffle('x')])
                        j+=1
                    if i in range(9,14):
                        tt=T.concatenate([x_t[24*i:24*(i+2)],x_t[24*(7+i):24*(9+i)]])
                        h_tm=T.concatenate([h_tm,T.dot(tt,self.W_in4[:,k*32+j]).dimshuffle('x')])
                        j+=1
                    if i in range(15,21):
                        tt=T.concatenate([x_t[24*i:24*(i+2)],x_t[24*(8+i):24*(10+i)]])
                        h_tm=T.concatenate([h_tm,T.dot(tt,self.W_in4[:,k*32+j]).dimshuffle('x')])
                        j+=1
                    if i in range(22,29):
                        tt=T.concatenate([x_t[24*i:24*(i+2)],x_t[24*(8+i):24*(10+i)]])
            ...
            for k in range(0,n_mul):
                ...
                for i in range(0,46):
                    if i in range(0,2):
                        tt=T.concatenate([x_t[24*i:24*(i+3)],x_t[24*(5+i):24*(8+i)]])
                        tt=T.concatenate([tt,x_t[24*(11+i):24*(14+i)]])
                        h_tm=T.concatenate([h_tm,T.dot(tt,self.W_in9[:,k*20+j]).dimshuffle('x')])
                        j+=1
                    if i in range(4,7):
                        tt=T.concatenate([x_t[24*i:24*(i+3)],x_t[24*(6+i):24*(9+i)]])
                        tt=T.concatenate([tt,x_t[24*(13+i):24*(16+i)]])
                        h_tm=T.concatenate([h_tm,T.dot(tt,self.W_in9[:,k*20+j]).dimshuffle('x')])
                        j+=1
                    if i in range(9,13):
                        tt=T.concatenate([x_t[24*i:24*(i+3)],x_t[24*(7+i):24*(10+i)]])
                        tt=T.concatenate([tt,x_t[24*(15+i):24*(18+i)]])
                        h_tm=T.concatenate([h_tm,T.dot(tt,self.W_in9[:,k*20+j]).dimshuffle('x')])
                        j+=1
                    if i in range(15,20):
                        tt=T.concatenate([x_t[24*i:24*(i+3)],x_t[24*(8+i):24*(11+i)]])
                        tt=T.concatenate([tt,x_t[24*(16+i):24*(19+i)]])
                        h_tm=T.concatenate([h_tm,T.dot(tt,self.W_in9[:,k*20+j]).dimshuffle('x')])
                        j+=1
                    if i in range(22,28):
                        tt=T.concatenate([x_t[24*i:24*(i+3)],x_t[24*(8+i):24*(11+i)]])
            ...
            for k in range(0,n_mul):
                ...
                for i in range(0,46):
                    if i in range(0,1):
                        tt=T.concatenate([x_t[24*i:24*(i+4)],x_t[24*(5+i):24*(9+i)]])
                        tt=T.concatenate([tt,x_t[24*(11+i):24*(15+i)]])
                        tt=T.concatenate([tt,x_t[24*(18+i):24*(22+i)]])
                        h_tm=T.concatenate([h_tm,T.dot(tt,self.W_in16[:,k*10+j]).dimshuffle('x')])
                        j+=1
                    if i in range(4,6):
                        tt=T.concatenate([x_t[24*i:24*(i+4)],x_t[24*(6+i):24*(10+i)]])
                        tt=T.concatenate([tt,x_t[24*(13+i):24*(17+i)]])
                        tt=T.concatenate([tt,x_t[24*(21+i):24*(25+i)]])
                        h_tm=T.concatenate([h_tm,T.dot(tt,self.W_in16[:,k*10+j]).dimshuffle('x')])
                        j+=1
                    if i in range(9,12):
                        tt=T.concatenate([x_t[24*i:24*(i+4)],x_t[24*(7+i):24*(11+i)]])
                        tt=T.concatenate([tt,x_t[24*(15+i):24*(19+i)]])
                        tt=T.concatenate([tt,x_t[24*(23+i):24*(27+i)]])
                        h_tm=T.concatenate([h_tm,T.dot(tt,self.W_in16[:,k*10+j]).dimshuffle('x')])
                        j+=1
                    if i in range(15,19):
                        tt=T.concatenate([x_t[24*i:24*(i+4)],x_t[24*(8+i):24*(12+i)]])
                        tt=T.concatenate([tt,x_t[24*(16+i):24*(20+i)]])
                        tt=T.concatenate([tt,x_t[24*(24+i):24*(28+i)]])
                        h_tm=T.concatenate([h_tm,T.dot(tt,self.W_in16[:,k*10+j]).dimshuffle('x')])
                        j+=1
           
              '''
        def step(x_t, h_tm1):
            h_t = self.activation(T.dot(x_t,mark*self.W_in1)+ \
        ...
        self.L1 += abs((mark*self.W_in1).sum())
        #self.L1 += abs(self.W_in4.sum())
        #self.L1 += abs(self.W_in9.sum())
        #self.L1 += abs(self.W_in16.sum())
        ...
        self.L2_sqr += ((mark*self.W_in1) ** 2).sum()
        #self.L2_sqr += (self.W_in4 ** 2).sum()
        #self.L2_sqr += (self.W_in9 ** 2).sum()
        #self.L2_sqr += (self.W_in16 ** 2).sum()
...
class MetaRNN(BaseEstimator):
    def __init__(self, n_in=5, n_mul=2, n_out=5, learning_rate=0.01,
    ...
    def ready(self):
        ...
        self.rnn = RNN(input=self.x, n_in=self.n_in,
                       n_mul=self.n_mul, n_out=self.n_out,
    ...
    def fit(self, X_train, Y_train, X_test=None, Y_test=None,
        ...
        while (epoch < self.n_epochs):
            #print epoch, self.n_epochs, n_train
            epoch = epoch + 1
            for idx in xrange(n_train):
                ...
                if iter % validation_frequency == 0:
                    ...
                    if self.interactive:
                        ...
                        print epoch, idx + 1, n_train, this_train_loss, self.learning_rate
