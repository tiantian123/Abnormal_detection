...
train :
  ...
  h: Theano variable or None
      ...
      of an RNN).'''
    print 'CRC: Initial HF', datetime.datetime.now().time()
  ...
  def cg(self, b):
    print '\n CRC: Begin CG', datetime.datetime.now().time()
    print '\n CRC: Begin CG size of b', len(b)    
    ...
    print '\n CRC: CG Begin GV', datetime.datetime.now().time()
    r = b - self.batch_Gv(x)
    print '\n CRC: CG End GV', datetime.datetime.now().time()
    ...
    for i in xrange(1, 1 + self.max_cg_iterations):
      print '\n CRC: CG iteration: ', i, datetime.datetime.now().time()
  ...
  def train(self, gradient_dataset, cg_dataset, initial_lambda=500, mu=0.01, global_backtracking=False, preconditioner=False, max_cg_iterations=500, num_updates=100, validation=None, validation_frequency=1, patience=numpy.inf, save_progress=None):
  ...
  save_progress: string or None
    ...
    print '\n CRC: Begin HF train', datetime.datetime.now().time()
    ...
    try:
      for u in xrange(first_iteration, 1 + num_updates):
        print '\n CRC: HF train iter: ', u, datetime.datetime.now().time()
        ...
        print '\n CRC: HF train iter: ', u, 'Begin CG', datetime.datetime.now().time()
        after_cost, flat_delta, backtracking, num_cg_iterations = self.cg(-gradient)# crc begin CG
        print '\n CRC: HF train iter: ', u, 'End CG', datetime.datetime.now().time()
