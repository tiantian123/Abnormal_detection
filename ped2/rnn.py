# -*- coding: utf-8 -*-
"""Created on Tue Dec 22 09:35:08 2015
@author: TianChen
"""
import numpy as np
import theano
...
class RNN(object):
    ...
    def __init__(self, input, n_in, n_mul, n_out, activation=T.tanh,
        ...
        W_init = np.asarray(np.random.uniform(size=(n_mul, n_mul),
                                               low=-.01, high=.01),
                                               dtype=theano.config.floatX)
        self.W = theano.shared(value=W_init, name='W')
        # input to hidden layer weights
        W_in_init = np.asarray(np.random.uniform(size=(n_in, n_mul),
                                                 low=-.01, high=.01),
                                                 dtype=theano.config.floatX)
        ...
        W_out_init = np.asarray(np.random.uniform(size=(n_mul, n_out),
                                                  low=-.01, high=.01),
                                                  dtype=theano.config.floatX)
        self.W_out = theano.shared(value=W_out_init, name='W_out')
        
        h0_init = np.zeros((n_mul,), dtype=theano.config.floatX)
        self.h0 = theano.shared(value=h0_init, name='h0',borrow=True)
       
        bh_init = np.zeros((n_mul,), dtype=theano.config.floatX)
...
class MetaRNN(BaseEstimator):
    def __init__(self, n_in=5, n_mul=2, n_out=5, learning_rate=0.01,
                 ...
                 activation='tanh', output_type='real',index=0,
    ...
    def fit(self, X_train, Y_train, X_test=None, Y_test=None,
        ...
        path='/root/chentian/share/201605/ped2/param'
        os.makedirs(path)
        path_W_in=path+'/W_in.csv'
        fw_in=file(path_W_in,"a")
        np.savetxt(fw_in,self.rnn.W_in.get_value(),delimiter=",")
        fw_in.close()
        
        path_W = path+'/W.csv'
        fW = file(path_W,"a")
        np.savetxt(fW,self.rnn.W.get_value(),delimiter=",")
        fW.close()
        
        path_W_out=path+'/W_out.csv'
        fW_out = file(path_W_out,"a")
        np.savetxt(fW_out,self.rnn.W_out.get_value(),delimiter=",")
        fW_out.close()
        
        path_h=path+'/h0.csv'
        f1=file(path_h,"a")
        np.savetxt(f1,self.rnn.h0.get_value(),delimiter=",")
        f1.close()
        
        path_bh=path+'/bh.csv'
        f2=file(path_bh,"a")
        np.savetxt(f2,self.rnn.bh.get_value(),delimiter=",")
        f2.close()
        
        path_by=path+'/by.csv'
        f3=file(path_by,"a")
        np.savetxt(f3,self.rnn.by.get_value(),delimiter=",")
        f3.close()
